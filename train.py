"""
TruthLens v4.0 — Fine-Tuning Training Script
=============================================
...
Run:
  python train.py --data ./data --epochs 20 --batch 8 --lr 1e-4
"""
"""
TruthLens v4.0 — Training Script (Fixed)
Run:
  python train.py --data ./data --epochs 15 --batch 32 --lr 2e-4 --workers 2
"""
...
def validate(model, loader, criterion, device):
    ...
    all_probs  = np.nan_to_num(
        np.array(all_probs,  dtype=np.float32), nan=0.5
    )
    all_preds  = np.array(all_preds,  dtype=np.int32)
    all_labels = np.nan_to_num(
        np.array(all_labels, dtype=np.float32), nan=0.0
    )
    ...
    return total_loss / len(loader), correct / total, auc, f1
...
def main():
    ...
    print("\n" + "="*55)
    print("  TruthLens v4.0 — Training Script (Fixed)")
    print("="*55)
    ...
    train_ds = datasets.ImageFolder(
        train_path, transform=get_transforms('train')
    )
    val_ds = datasets.ImageFolder(
        val_path, transform=get_transforms('val')
    )
    ...
    sampler = WeightedRandomSampler(
        weights, len(train_ds), replacement=True
    )
    ...
    print("Phase 1: Backbone frozen (epochs 1-3)")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch == 4:
            ...
            optimiser.param_groups[2]['lr'] = args.lr * 0.05
